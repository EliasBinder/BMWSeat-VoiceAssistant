...
export async function interpretMessage(userInput: string){
    const response = await openAI.openai.createChatCompletion({
        ...
        messages: [
            {role: "system", content: "You are a helpful assistant that can move a car seat. You can move the seat forward, backward, up, down, and adjust the backrest. The user may not express his request directly, but instead may express the current circumstances which he is not happy with."},
